...
export type QuestionBank = [document: string, numbering: string];

// {Questions: {Question: {title, content: ReactNode, ...}, ...}, ...}
export type ParsedQuestionBank = {};

export const blobToQuestionBank: (blob: Blob) => Promise<QuestionBank> = async (blob: Blob) => {
    try {
        const zip = new JSZip();
...
export const parseQuestionBank: (questionBank: QuestionBank) => Promise<ParsedQuestionBank> = async (questionBank: QuestionBank) => {
    if (!questionBank)
        return {};
    ...
    const newHeading3 = (fullText: string) => {
        if (currentHeading1 && currentHeading2) {
            if (currentStyle && currentStyle !== 'Heading1' && currentStyle !== 'Heading2')
                finaliseCurrentHeading3();

            let title: string = fullText;
            while (output[currentHeading1][currentHeading2].hasOwnProperty(fullText))
                title = '_' + title;
    ...
    const getCurrentContentAsWrappedReactNode: () => ReactNode = () => {
        ...
        return currentFormat
            ? currentFormat === 'bullet'
                ? <ul>{node}</ul>
                : <ol type={convertFormatToType(currentFormat)}>{node}</ol>
            : node;
    }
    ...
    for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (nextStyle === 'Subtitle') {
            ...
            let title: string = fullText;
            while (output.hasOwnProperty(fullText))
                title = '_' + title;
            ...
            if (currentHeading1) {
                if (currentStyle && currentStyle !== 'Heading1')
                    finaliseCurrentHeading3();

                let title: string = fullText;
                while (output[currentHeading1].hasOwnProperty(fullText))
                    title = '_' + title;
        ...
        currentStyle = nextStyle ? nextStyle : undefined;
    }
